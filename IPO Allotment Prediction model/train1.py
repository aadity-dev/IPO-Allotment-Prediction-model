...
def create_features(df):
    ...
    if 'Issue Size  (in crores)' in df.columns:
        df['Log_Issue_Size'] = np.log1p(df['Issue Size  (in crores)']) #Creates Log_Issue_Size using the log-plus-one transformation ln(x+1) to normalize the distribution of the original feature.
...
#1. Fills missing values with the median (SimpleImputer). 2. Standardizes the data (mean=0, variance=1) (StandardScaler).
    preprocessor = ColumnTransformer([#Creates a ColumnTransformer to combine the two pipelines defined above.
        ('num', numerical_pipeline, numerical_features),#Applies the numerical_pipeline to the numerical_features
        ('cat', categorical_pipeline, categorical_features)
    ])
#Transforms the test data X_test using the statistics learned from the training data (prevents data leakage).
    X_train_processed = preprocessor.fit_transform(X_train)
    X_test_processed = preprocessor.transform(X_test)
    
    smote = SMOTE(random_state=42) # intializes SMOTE object for over-sampling.
    ...
    # Define hyperparameter grids for each model and Initializes a dictionary param_grids containing parameter ranges for tuning each model
    param_grids = {
        'Random Forest': {
            'n_estimators': [100, 200],#Number of trees in the forest. More trees can improve accuracy but take longer to train.
            'max_depth': [10, 20, None],#Maximum depth of each tree. Lower values prevent overfitting; None means unlimited depth.
            'min_samples_split': [2, 5]#Minimum samples needed to split a node. Higher values make trees more conservative.
        },
        'XGBoost': {
            'n_estimators': [100, 200],#Number of boosting rounds 
            'learning_rate': [0.01, 0.1],#Step size shrinkage used in updates. Lower values make learning slower but can improve accuracy.
            'max_depth': [3, 5, 7]#Maximum depth of each tree. Controls model complexity.
        },
        'Logistic Regression': {
            'C': [0.1, 1.0, 10.0],#Lower values mean stronger regularization (simpler model).
            'solver': ['liblinear', 'lbfgs']#solvers work better for certain data sizes or regularization types.
        }
    }

    best_model_pipeline = None #o store the best performing model pipeline.
    best_score = 0 #to get highesr roc auc socre
    best_model_name = ""
    ...
    for name, model in models.items():
        ...
        # NOTE: GridSearchCV will be performed on the preprocessed and resampled data #sing 5-fold cross-validation (cv=5)
        grid_search = GridSearchCV(model, param_grids[name], cv=5, scoring='roc_auc', n_jobs=-1)
        grid_search.fit(X_train_resampled, y_train_resampled) #Executes the grid search, fitting the model on the SMOTE-resampled and preprocessed training data.
    ...
    calibrated_pipeline = CalibratedClassifierCV(best_model_pipeline, method='isotonic', cv=5)# 5-fold cross-validation (cv=5)
